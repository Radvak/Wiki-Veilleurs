---
description: Coordonnées (3234,1484)
---

# 🔨 Wali l'Apprenti

***

<h3 align="center">Difficulté ⭐▪️▪️▪️▪️</h3>

***

#### Prérequis pour commencer et rendre la mission

* Avoir terminé "[Méphisto](mephisto.md)"

#### Tâche :&#x20;

{% stepper %}
{% step %}
Parler à Wali

{% hint style="info" %}
Coordonnées (2380,2411)
{% endhint %}

{% hint style="warning" %}
L'Apprenti: Wali, aura changé d'emplacement à partir de cette quête. Il se trouvera alors dans la Bibliothèque juste à côté de Méphisto (3195,1497)
{% endhint %}
{% endstep %}

{% step %}
Parlez à Émy

{% hint style="info" %}
Émy se trouve au niveau des quartiers de la milice de Tolbana, plus précisément dans la Forge, là où se trouve les Forgerons d'Armes et d'Armure (3234,1484)
{% endhint %}
{% endstep %}
{% endstepper %}

#### Récompenses

* Ø
